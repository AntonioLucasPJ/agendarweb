export function Appointment(props) {
    ...
    return <tr>
        <td scope="row">{props.id_appointement}</td>
        <td scope="row">{props.mecanico}</td>
        <td scope="row">{props.service}</td>
        <td scope="row">{new Intl.DateTimeFormat('pt-BR', { dateStyle: 'short' }).format(convertedt)}</td>
        ...
        <td className="text-end">
            ...
            <button className="btn btn-sm btn-danger" onClick={() => props.clickdelete(props.id_appointement,props.mecanico,props.service)}><i className="bi bi-trash3"></i></button>
